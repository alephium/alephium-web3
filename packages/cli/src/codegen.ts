...
const oneAlph = `BigInt(${ONE_ALPH.toString()})`
...
function genDeploy(contract: Contract): string {
  ...
  const fieldsParam = getParamsFromFieldsSig(contract.fieldsSig, `${contract.name}.Fields`)
  return `
  export async function deploy(signer: SignerProvider, ${fieldsParam}${deployParams}): Promise<Contract> {
    ...
    return new Contract(deployResult.contractAddress, deployResult.contractId, deployResult.fromGroup, deployResult)
  }
  `
}

function genAttach(): string {
  return `
  export function attach(address: string, deployResult?: SignDeployContractTxResult): Contract {
...
function genSubscribeEvent(contractName: string, event: EventSig, eventIndex: number): string {
  ...
  return `
    ...
    subscribe${eventType}(options: SubscribeOptions<${eventType}>, fromCount?: number): EventSubscription {
...
function genStateForTest(contract: Contract): string {
  ...
  return `
    ...
    static stateForTest(${formatParameters(contract.fieldsSig)}asset?: Asset, address?: string): ContractState {
      ...
      return artifact.toState({${initialFields}}, newAsset, address)
    }
  `
}
...
function genTestMethod(contract: Contract, functionSig: node.FunctionSig, index: number): string {
  ...
  const fieldParam = getParamsFromFieldsSig(contract.fieldsSig, `${contract.name}.Fields`)
  ...
  return `
    static async test${funcName}Method(${funcArgs}${fieldParam}${testParams}): Promise<${retType}> {
      const initialAsset = {
