...
import { Project, SourceInfo } from './project'
...
function genContract(contract: Contract): string {
  ...
  return `
    ...
    import { ${contract.name}ContractArtifact, getContractByCodeHash } from './contracts'
    ${importStructs()}
    ...
    export const ${contract.name} = new Factory(${contract.name}ContractArtifact)
...
function genContractByCodeHash(
  contracts: { sourceInfo: SourceInfo; artifact: Contract }[],
  outDir: string,
  artifactDir: string
) {
  const projectArtifact = currentProject.projectArtifact
  const imports = contracts.map((c) => {
    const artifactPath = c.sourceInfo.getArtifactPath(artifactDir)
    const artifactRelativePath = path.relative(artifactDir, artifactPath)
    return `import { default as ${c.artifact.name}ContractJson } from '../${toUnixPath(artifactRelativePath)}'`
  })
  const defs = contracts.map((c) => {
    const contractInfo = projectArtifact.infos.get(c.artifact.name)
    if (contractInfo === undefined) {
      throw new Error(`Contract info does not exist: ${c.artifact.name}`)
    }
    return `export const ${c.artifact.name}ContractArtifact: Contract =
      Contract.fromJson(
        ${c.artifact.name}ContractJson,
  ...
  const allContracts = contracts.map((c) => `${c.artifact.name}ContractArtifact`)
  const source = `
    ${header}
    ...
    export function getContractByCodeHash(codeHash: string): Contract {
      if (contractArtifacts === undefined) {
        contractArtifacts = [${allContracts.join(', ')}]
      }
...
function genContracts(contracts: { sourceInfo: SourceInfo; artifact: Contract }[], outDir: string, exports: string[]) {
  contracts.forEach((c) => {
...
export function codegen(project: Project) {
  ...
  try {
    ...
    const sortedContracts = sortByName(Array.from(currentProject.contracts.values()))
    genContracts(sortedContracts, outDir, exports)
    genContractByCodeHash(sortedContracts, outDir, artifactDir)
